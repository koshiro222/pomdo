...
## Current Milestone: v1.6.2 Statsカードデザイン改善

**Goal:** Statsカードのグラフを見やすく改善し、曜日表示を日曜始まりにする

**Target features:**
- 既存グラフの改善（サイズ、余白、ラベル等の調整）
- 曜日の並びを日曜始まりに変更

## Current State

**Latest Milestone:** v1.6.2 Statsカードデザイン改善 (completed 2026-03-27)

**Shipped v1.6.1:**
- Phase 19: BGMプレイヤーアニメーション基盤 — Validated in Phase 19
  - blink @keyframes（opacity 0.6-1.0、scale 1-1.02、2s周期）
  - pulse-glow @keyframes（3層box-shadow、currentColor対応）
  - album-art-spinning削除、album-art-paused維持
- Phase 20: AlbumArtコンポーネント修正 — Validated in Phase 20
  - AlbumArtへのアニメーションクラス適用（album-art-blinking, album-art-pulsing）
  - 再生中のみ点滅+パルスアニメーション、停止時は静止
  - album-art-spinningクラスの完全削除

**Shipped v1.6.2:**
- Phase 21: 曜日表示を日曜始まりに変更 — Validated in Phase 21
  - getDayLabel()関数の日曜始まり実装
  - 週データ生成ロジックを日曜始まりに修正
- Phase 22: グラフ視認性向上 — Validated in Phase 22
  - 動的aspect比計算（containerWidth * 0.4）によるグラフサイズ調整
  - shadcn/ui準拠のミニマル軸スタイル（tickLine/axisLine非表示、stroke="#9ca3af"）
  - Barコンポーネントの角丸（radius={8}）
  - accessibilityLayerによるキーボードナビゲーションとARIA対応
...
- ✓ BGMプレイヤーアニメーション基盤 — v1.6.1 Phase 19で実装
  - ✓ blink @keyframes（opacity 0.6-1.0、scale 1-1.02、2s周期）
  - ✓ pulse-glow @keyframes（3層box-shadow、currentColor対応）
  - ✓ album-art-spinning削除、album-art-paused維持
- ✓ BGMプレイヤーコンポーネント修正 — v1.6.1 Phase 20で実装
  - ✓ AlbumArtへのアニメーションクラス適用（album-art-blinking, album-art-pulsing）
  - ✓ 再生中のみ点滅+パルスアニメーション、停止時は静止
  - ✓ album-art-spinningクラスの完全削除
- ✓ 曜日表示を日曜始まりに変更 — v1.6.2 Phase 21で実装
  - ✓ getDayLabel()関数の日曜始まり実装
  - ✓ 週データ生成ロジックを日曜始まりに修正
- ✓ Statsカードグラフ改善 — v1.6.2 Phase 22で実装
  - ✓ 動的aspect比計算によるグラフサイズ調整
  - ✓ shadcn/ui準拠のミニマル軸スタイル（tickLine/axisLine非表示）
  - ✓ text-cf-subtext色の軸ラベル
  - ✓ Barコンポーネントの角丸（radius={8}）
  - ✓ accessibilityLayerによるキーボードナビゲーション対応
...
| CSS keyframesのみ使用 | Framer Motion不使用、CSSでアニメーション完結 | ✓ Good — v1.6.1で実装完了 |
| 光感受性エピレプシー対策 | 2s周期以上、opacity min 0.4、パルスエフェクト採用 | ✓ Good — v1.6.1で実装完了 |
| currentColor継承パターン | アルバムアートの色を動的に継承し、テーマ対応 | ✓ Good — v1.6.1で実装完了 |

---
*Last updated: 2026-03-27 after v1.6.2 milestone completion*
