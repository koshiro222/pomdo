# Requirements Archive: v1.0 BGM管理機能追加
...
- [x] **API-04**: `update` ミューテーション — 管理者のみ、トラック更新
- [x] **API-05**: `delete` ミューテーション — 管理者のみ、トラック削除
- [x] **API-06**: R2へのファイルアップロード API
- [x] **API-07**: R2からのファイル削除 API
- [x] **API-08**: admin ロール判定ミドルウェア

### Frontend - Player

- [x] **FE-01**: `useBgm` フックをDBから取得するように変更
- [x] **FE-02**: ハードコードされた `TRACKS` 定数を削除
- [x] **FE-03**: tRPC `bgm.getAll` でトラック取得
- [x] **FE-04**: エラーハンドリング（BGM取得失敗時）
...
- [x] **AUTH-01**: Better Auth で admin ロール設定方法確認
- [x] **AUTH-02**: 管理者判定ロジック実装
- [x] **AUTH-03**: 非管理者の管理APIアクセスを拒否（TRPCError）
