...
- [ ] **DB-01**: bgm_tracks テーブルを作成（id, title, artist, color, filename, tier, createdAt, updatedAt）
- [ ] **DB-02**: Drizzleスキーマ定義を作成
- [ ] **DB-03**: マイグレーションファイル生成と適用

### Backend API

- [ ] **API-01**: tRPC ルーター `bgm` を作成
- [ ] **API-02**: `getAll` クエリ — 全トラック取得（tierでフィルタ可能）
- [ ] **API-03**: `create` ミューテーション — 管理者のみ、トラック追加
- [ ] **API-04**: `update` ミューテーション — 管理者のみ、トラック更新
- [ ] **API-05**: `delete` ミューテーション — 管理者のみ、トラック削除
- [ ] **API-06**: R2へのファイルアップロード API
- [ ] **API-07**: R2からのファイル削除 API
...
- [ ] **UI-01**: Headerに管理ボタン（adminロールのみ表示）
- [ ] **UI-02**: BGM管理モーダル/ページ
- [ ] **UI-03**: トラック一覧表示（テーブル）
- [ ] **UI-04**: ファイルアップロード機能（ドラッグ＆ドロップ対応）
- [ ] **UI-05**: 曲名・アーティスト・色編集フォーム
- [ ] **UI-06**: 削除確認ダイアログ
